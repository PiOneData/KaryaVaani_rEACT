...
'Regards,\nKarya Vaani (on behalf of Daikin Sricity HR)';
  ...
  function openVendorEmail(workerName) {
    ...
    _emailCtx = c;
    // derive a WhatsApp number for the vendor (the directory carries no phone);
    // KVWhatsApp falls back to the configured test recipient if unusable.
    const vh = (typeof omHash === 'function') ? omHash(w.contractor) : 0;
    _emailCtx.vendorPhone = '91' + ('9' + String(100000000 + (vh % 900000000)));
  ...
  /* send the vendor notice through the VAANI mailer (real /api/send-email) */
  function sendEmail() {
    if (!_emailCtx) return;
    const to = _emailCtx.vendor.email;
    const cc = [_emailCtx.vendor.cc, CC_INTERNAL].filter(Boolean);
    const subject = document.getElementById('em-subject').value;
    const message = document.getElementById('em-body').value;
    toast('Sending email…', 'green');
    fetch((window.__KV_API_BASE || '') + '/api/send-email', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ to: [to], cc: cc, subject: subject, message: message })
    }).then(function (r) { return r.json().catch(function () { return {}; }); })
      .then(function (j) {
        if (j && j.ok) toast('Email sent to ' + _emailCtx.vendor.contact + ' · ' + to, 'green');
        else toast('Email failed: ' + ((j && j.error) || 'unknown error'), 'red');
        closeEmailModal();
      })
      .catch(function (e) { toast('Email failed: ' + e.message, 'red'); });
  }

  /* send the same notice over WhatsApp through the gateway (real KVWhatsApp) */
  function waSendVendor() {
    if (!_emailCtx) return;
    const phone = _emailCtx.vendorPhone || window.__KV_WHATSAPP_TEST_TO || '';
    const message = document.getElementById('em-body').value;
    if (!window.KVWhatsApp) { toast('WhatsApp gateway unavailable', 'red'); return; }
    if (!phone) { toast('No WhatsApp number for this vendor', 'red'); return; }
    toast('Sending WhatsApp…', 'green');
    window.KVWhatsApp.notify(phone, message, { label: 'WhatsApp sent to ' + _emailCtx.vendor.contact })
      .then(function () { closeEmailModal(); })
      .catch(function (e) { toast('WhatsApp failed: ' + (e.message || e), 'red'); });
  }
  /* kept for compatibility — the modal button now sends via WhatsApp, not Gmail */
  function openInGmail() { waSendVendor(); }
  ...
  const KC_TRANSLATIONS = {
    ...
    wages: {
      TE: '2026 సంవత్సరానికి AP కనీస వేతనాలు: నైపుణ్యం లేని కార్మికులు ₹13,210/నెల · సెమీ-స్కిల్డ్ కార్మికులు ₹14,260/నెల · స్కిల్డ్ కార్మికులు ₹15,860/నెల · అత్యంత నైపుణ్యం గల కార్మికులు ₹17,290/నెల. వీటిలో VDA కలిసి ఉంది. తాజా సవరణ నోటీసు నోటీసుల విభాగంలో ఉంది.',
      HI: '2026 के लिए AP न्यूनतम वेतन: अकुशल श्रमिक ₹13,210/माह · अर्ध-कुशल श्रमिक ₹14,260/माह · कुशल श्रमिक ₹15,860/माह · अति-कुशल श्रमिक ₹17,290/माह। इनमें VDA शामिल है। नवीनतम संशोधन नोटिस, नोटिस अनुभाग में लगा है।',
      TA: '2026-க்கான AP குறைந்தபட்ச ஊதியம்: திறனற்ற தொழிலாளர் ₹13,210/மாதம் · அரை-திறன் தொழிலாளர் ₹14,260/மாதம் · திறன் தொழிலாளர் ₹15,860/மாதம் · உயர்-திறன் தொழிலாளர் ₹17,290/மாதம். இவற்றில் VDA அடங்கும். சமீபத்திய திருத்த அறிவிப்பு அறிவிப்புகள் பகுதியில் வெளியிடப்பட்டுள்ளது.',
      OR: '2026 ପାଇଁ AP ସର୍ବନିମ୍ନ ମଜୁରି: ଅଦକ୍ଷ ଶ୍ରମିକ ₹13,210/ମାସ · ଅର୍ଦ୍ଧ-ଦକ୍ଷ ଶ୍ରମିକ ₹14,260/ମାସ · ଦକ୍ଷ ଶ୍ରମିକ ₹15,860/ମାସ · ଅତି-ଦକ୍ଷ ଶ୍ରମିକ ₹17,290/ମାସ। ଏଥିରେ VDA ଅନ୍ତର୍ଭୁକ୍ତ। ସର୍ବଶେଷ ସଂଶୋଧନ ନୋଟିସ୍ ନୋଟିସ୍ ବିଭାଗରେ ଅଛି।',
      BN: '2026 সালের জন্য AP ন্যূনতম মজুরি: অদক্ষ শ্রমিক ₹13,210/মাস · আধা-দক্ষ শ্রমিক ₹14,260/মাস · দক্ষ শ্রমিক ₹15,860/মাস · অতি-দক্ষ শ্রমিক ₹17,290/মাস। এতে VDA অন্তর্ভুক্ত। সর্বশেষ সংশোধন নোটিশ নোটিশ বিভাগে রয়েছে।',
      JP: '2026年のAP州最低賃金:未熟練労働者 月₹13,210 · 半熟練労働者 月₹14,260 · 熟練労働者 月₹15,860 · 高度熟練労働者 月₹17,290。これらにはVDAが含まれます。最新の改定通知は通知欄に掲示されています。',
    },
    leave: {
  ...
  function kcRenderDocBrowser() {
    ...
    docs.forEach(d => {
      ...
      cards += '<div class="kc-doc-card' + (isActive ? ' active' : '') + '" onclick="kcSelectDoc(\'' + d.id + '\')">' +
               ...
               (d.body ? '<span class="kc-doc-card-read" onclick="event.stopPropagation();kcDownloadDocx(\'' + d.id + '\')" title="Download as Word (.docx)">⤓ .docx</span>' : '') +
  ...
  function kcCannedAnswer(question) {
    ...
    if (/bus|transport|pickup|route|drop|commute/i.test(q)) {
      ...
      answerText = 'AP minimum wages for 2026: Unskilled labour ₹13,210/mo · Semi-skilled labour ₹14,260/mo · Skilled labour ₹15,860/mo · Highly skilled labour ₹17,290/mo. These include VDA. The latest revision notice is posted under Notices.';
  ...
  function kcReadDoc(docId) {
    ...
    omModal(
      ...
      '<div class="modal-footer"><div class="modal-footer-left"><span class="tiny muted">Version-controlled document · Knowledge Center</span></div>' +
        ...
        (d.body ? '<button class="btn" onclick="kcDownloadDocx(\'' + d.id + '\')">⤓ Download .docx</button>' : '') +
        '<button class="btn primary" onclick="omCloseModal();kcSelectDoc(\'' + d.id + '\')">Use in chat / translate</button>' +
  ...
  /* download the document as a Word (.docx) file — served from the repo at
     /legacy/docs/<id>.docx (generated by scripts/gen_docx.cjs). */
  function kcDownloadDocx(docId) {
    const d = kcDocById(docId);
    if (!d) return;
  ...
  const VB_KC_TEMPLATES = {
    'kc-301': {
      subject: 'Minimum wage revision · 2026',
      body: 'The 2026 minimum wage revision is now notified. AP rates: unskilled labour ₹13,210, semi-skilled labour ₹14,260, skilled labour ₹15,860, highly skilled labour ₹17,290 per month, inclusive of VDA. The revision notice is posted on the notice board. Your next pay slip reflects the new rate. Contact Plant HR with any questions.'
    },
    'kc-305': {
  ...
  /* ════════════════════════════════════════════════════════════════
     TRANSPORT OPERATIONS BOARD (GoKarya layout)
     Shift tabs (A morning / B general / C night) · compact route cards with an
     EMP-vs-contract split · a roster drawer that expands under the grid when a
     route is clicked; each passenger opens the full employee detail. Night shift
     surfaces the OSHC Rule-83 women-transport compliance strip + SOS.
     ════════════════════════════════════════════════════════════════ */
  const TR_SHIFT_META = {
    A: { key: 'morning', label: 'Shift A · morning transport', night: false },
    B: { key: 'general', label: 'Shift B · general transport', night: false },
    C: { key: 'night',   label: 'Shift C · night transport',   night: true }
  };
  function trTxt(id, v) { const e = document.getElementById(id); if (e) e.textContent = v; }
  function trGkBatch(routeCode, s) {
    if (s === 'C') return trRoster().filter(function (a) { return a.route === routeCode && a.gender === 'F'; });
    const key = (s === 'A') ? 'morning' : 'general';
    return trRoster().filter(function (a) { return a.route === routeCode && a.shift === key; });
  }
  function trGkBoardTime(route, s) {
    if (s === 'C') return '22:15';
    return ((route[(s === 'A') ? 'morning' : 'general']) || {}).board || '—';
  }
  function trTown(r) { return (r.route.indexOf('·') !== -1 ? r.route.split('·')[1] : r.zone).trim(); }

  function trShift(s) {
    TR_STATE.gkShift = s;
    TR_STATE.shift = (s === 'A') ? 'morning' : 'general';   // for the real notify/attendance endpoints
    ['A','B','C'].forEach(function (x) { const t = document.getElementById('tr-tab-' + x); if (t) t.classList.toggle('on', x === s); });
    trGkRenderRoutes();
    trGkRenderSummary();
    trGkRenderNight();
  ...
  function trGkOpenRoster(code) {
    ...
    trTxt('tr-rd-ctr', (batch.length - emp) + ' contract workers');
    trTxt('tr-rd-rate', boarded + '/' + batch.length + ' boarded (' + (batch.length ? Math.round(boarded / batch.length * 100) : 0) + '%)');
    const tb = document.getElementById('tr-rd-tbody');
    if (tb) tb.innerHTML = batch.length ? batch.map(function (a) {
      const brd = trTodayBrd(a.code);
      const typeBadge = a.type === 'emp' ? '<span class="type-emp">EMP</span>' : '<span class="type-ctr">CTR</span>';
  ...
  function trGkCloseRoster() { const d = document.getElementById('tr-roster-drawer'); if (d) d.classList.remove('open'); TR_STATE.gkOpenRoute = null; }

  function trGkRenderSummary() {
    const s = TR_STATE.gkShift || 'A';
    trTxt('tr-summary-label', TR_SHIFT_META[s].label);
    const inShift = TR_ROUTES.reduce(function (n, r) { return n + trGkBatch(r.code, s).length; }, 0);
    const kpis = (s === 'C')
      ? [{ v: TR_ROUTES.length, l: 'Night routes', c: 'sv-gk' }, { v: inShift, l: 'Women · night transport', c: 'sv-rose' },
         { v: '4/4', l: 'Escort vehicles · R.83', c: 'sv-sage' }, { v: '1', l: 'Unaccounted', c: 'sv-ember' }]
      : [{ v: TR_ROUTES.length, l: 'Routes active', c: 'sv-gk' }, { v: inShift, l: 'Workers · shift ' + s, c: 'sv-gk' },
         { v: '1', l: 'Route delayed', c: 'sv-amber' }, { v: '1', l: 'Route critical', c: 'sv-ember' }];
    const g = document.getElementById('tr-summary-grid');
    if (g) g.innerHTML = kpis.map(function (k) { return '<div class="ssg"><div class="ssg-val ' + k.c + '">' + k.v + '</div><div class="ssg-lbl">' + k.l + '</div></div>'; }).join('');
  }
  function trGkRenderNight() {
  ...
  /* Karyavaani transport comms — representative multilingual log with playback. */
  const TR_KV_MSGS = [
    { type: 'kvt-c', bar: 'kbar-c', label: 'Consent · Rule 83', time: '21:30', to: 'Night-shift women · Shift C',
      text: 'నేటి రాత్రి షిఫ్ట్ C కోసం రవాణా ఏర్పాటు చేయబడింది. "అవును" పంపండి.', ch: 'WhatsApp · TE', ack: 'ak-ok', ackl: '✓ 21:33',
      speech: 'Shift C bus departs Gate 2. Please reply yes to confirm your night shift transport consent.' },
    { type: 'kvt-t', bar: 'kbar-t', label: 'Route delay · Naidupet', time: '05:45', to: 'Naidupet route batch',
      text: 'నైదుపేట రూట్ బస్సు 15 నిమిషాలు ఆలస్యంగా ఉంది. కొత్త రాక సమయం 06:15.', ch: 'WhatsApp · SMS', ack: 'ak-p', ackl: 'Sending…',
      speech: 'Naidupet route bus is delayed 15 minutes. New arrival time 6:15 AM.' },
    { type: 'kvt-s', bar: 'kbar-s', label: 'Welfare check · auto', time: '23:50', to: 'Not boarded · welfare',
      text: 'మీరు సురక్షితంగా ఉన్నారా? 1800-XXX-XXXX కి కాల్ చేయండి.', ch: 'WhatsApp · Voice', ack: 'ak-no', ackl: 'No response',
      speech: 'Are you safe? Please call our helpline for assistance.' },
    { type: 'kvt-t', bar: 'kbar-t', label: 'Safe drop · confirmed', time: '23:14', to: 'Route Tada · safe drop',
  ...
  function trGkRenderKV() {
    ...
    host.innerHTML = TR_KV_MSGS.slice(0, 3).map(function (m, i) {
      return '<div class="kvm"><div class="kvm-bar ' + m.bar + '"></div><div class="kvm-body">' +
        ...
        '<div class="kvm-ft"><span class="kvm-ch">' + m.ch + '</span>' +
  ...
  function trGkNotifyOpen() { if (TR_STATE.gkOpenRoute) trNotifyBatch(TR_STATE.gkOpenRoute); }
  function trGkAttendOpen() {
    const code = TR_STATE.gkOpenRoute;
    if (!code || typeof trTakeAttendance !== 'function') return;
    const p = trTakeAttendance(code);
    if (p && p.then) p.then(function () { trGkRenderRoutes(); if (TR_STATE.gkOpenRoute) trGkOpenRoster(code); });
  }

  function initTr() {
    if (!document.getElementById('tr-route-grid')) return;
  ...
  function capRenderRecent() {
    ...
    box.innerHTML = CAP_STATE.recent.slice(0, 10).map(function (r, i) {
      ...
      const aadFlag = r.aadhaarVerified
        ? ' <span class="pill green tiny">Aadhaar ✓</span>'
        : ' <span class="pill red tiny">Aadhaar pending</span>';
      return '<div class="cap-recent-row" style="cursor:pointer" onclick="obOpenCapture(' + i + ')" title="Open onboarding details">' +
        ...
        '<div class="cap-recent-main">' +
          '<div class="cap-recent-name">' + r.name + ' <span class="pill ' +
            ...
            (r.type === 'direct' ? 'Direct' : 'Contract') + '</span>' + aadFlag + '</div>' +
        ...
        (confirmed
          ...
          : '<span class="cap-recent-link" onclick="event.stopPropagation();capResend(' + i + ')">Resend link</span>') +
      '</div>';
    }).join('');
  }
  ...
  function kvTabModal(opts) {
    ...
    omModal(
      '<div class="modal-h"><div class="modal-h-left">' +
        '<span class="modal-h-eye">' + (opts.eyebrow || '') + '</span>' +
  ...
  function kvTabSwitch(btn, id) {
    ...
    if (!modal) return;
    modal.querySelectorAll('.tabs .tab').forEach(function (t) { t.classList.remove('on'); });
    btn.classList.add('on');
    modal.querySelectorAll('.kvtab-pane').forEach(function (p) { p.style.display = (p.getAttribute('data-pane') === id) ? 'block' : 'none'; });
  }
  function kvKV(k, v) {
    return '<div class="row-between" style="padding:8px 0;border-bottom:1px solid var(--line);font-size:0.85rem">' +
      '<span class="muted">' + k + '</span><span class="t-strong" style="text-align:right">' + (v || '—') + '</span></div>';
  }
  ...
  function omOpenWorker(code) {
    ...
    kvTabModal({
      eyebrow: 'Employee · ' + r.code + ' · ' + (c.status === 'compliant' ? 'compliant' : 'non-compliant'),
  ...
  /* ════════════════════════════════════════════════════════════════════════
     ONBOARDING · Aadhaar verify (reusable) + captured-profile storage + the
     tabbed onboarding-detail modal with compliance checks, linked to the OM
     compliance model and the real contractor records.
     ════════════════════════════════════════════════════════════════════════ */

  /* generic Aadhaar upload+validate modal that writes the verified number into
     a target input (and calls onDone). Reuses the OCR + Verhoeff validator. */
  let _KV_AAD_TARGET = null, _KV_AAD_DONE = null;
  function kvAadhaarVerify(targetId, onDone) {
    _KV_AAD_TARGET = targetId; _KV_AAD_DONE = onDone || null;
    omModal(
      '<div class="modal-h"><div class="modal-h-left"><span class="modal-h-eye">Aadhaar eKYC</span>' +
        '<span class="modal-h-title">Upload &amp; verify Aadhaar</span></div>' +
        '<span class="modal-h-close" onclick="omCloseModal()">Close ✕</span></div>' +
      '<div class="modal-body">' +
        '<div class="cap-hint" style="margin-bottom:10px">Upload the Aadhaar document — the number is read automatically and validated (UIDAI Verhoeff checksum), or type it. Only the last 4 digits are retained (DPDP Act 2023).</div>' +
        '<div class="cap-drop" onclick="document.getElementById(\'kv-aad-file\').click()">' +
          '<div class="cap-drop-ico">⬆</div><div class="cap-drop-t">Drop the Aadhaar image, or click to upload</div>' +
          '<div class="cap-drop-s" id="kv-aad-status">PNG / JPG of the Aadhaar card</div>' +
          '<input type="file" id="kv-aad-file" accept="image/*" style="display:none" onchange="kvAadhaarFile(this)"></div>' +
        '<div class="field" style="margin-top:12px"><label class="field-l">Aadhaar number</label>' +
          '<input class="input" id="kv-aad-num" inputmode="numeric" placeholder="12-digit Aadhaar" oninput="kvAadhaarCheck()"></div>' +
        '<div id="kv-aad-result" class="tiny" style="min-height:18px"></div>' +
      '</div>' +
      '<div class="modal-footer"><div class="modal-footer-left"><span class="tiny muted">Verhoeff-validated · masked storage</span></div>' +
        '<div class="modal-footer-right"><button class="btn primary" id="kv-aad-verify" disabled onclick="kvAadhaarSave()">Verify</button>' +
        '<button class="btn" onclick="omCloseModal()">Cancel</button></div></div>'
    );
  }
  function kvAadhaarCheck() {
    const inp = document.getElementById('kv-aad-num'), out = document.getElementById('kv-aad-result'), btn = document.getElementById('kv-aad-verify');
    const raw = (inp ? inp.value : '').replace(/\D/g, '');
    const ok = verhoeffValid(raw);
    if (out) out.innerHTML = !raw ? '' : ok
      ? '<span style="color:var(--green-dk)">✓ Valid Aadhaar · ' + maskAadhaar(raw) + '</span>'
      : '<span style="color:var(--red-dk)">✕ Invalid — checksum failed</span>';
    if (btn) btn.disabled = !ok;
    return ok;
  }
  function kvAadhaarFile(input) {
    const file = input && input.files && input.files[0]; if (!file) return;
    const status = document.getElementById('kv-aad-status');
    if (status) status.textContent = 'Reading document… (first run downloads the OCR engine, ~10s)';
    omAadhaarOcr(file).then(function (num) {
      if (num) { const inp = document.getElementById('kv-aad-num'); if (inp) inp.value = num; kvAadhaarCheck(); if (status) status.textContent = 'Read from document ✓'; toast('Aadhaar read from document', 'green'); }
      else { if (status) status.textContent = 'Could not read a valid number — type it manually'; toast('OCR found no valid Aadhaar — enter manually', 'amber'); }
    }).catch(function () { if (status) status.textContent = 'OCR unavailable — type the number manually'; toast('OCR unavailable — enter manually', 'amber'); });
  }
  function kvAadhaarSave() {
    const raw = (document.getElementById('kv-aad-num') || {}).value || '';
  ...
  function obWorkerCompliance(rec) {
    const h = omHash(rec.id || rec.name || '');
    const ct = (rec.employment && rec.employment.contractor)
  ...
  /* load persisted captures on init so they survive reloads */
  function obLoadCaptures() {
    fetch((window.__KV_API_BASE || '') + '/api/onboarding-captures')
      .then(function (r) { return r.json().catch(function () { return {}; }); })
      .then(function (j) {
        if (!j || !j.ok || !Array.isArray(j.captures) || !j.captures.length) return;
        const have = {};
        CAP_STATE.recent.forEach(function (x) { if (x.backendId) have[x.backendId] = 1; });
        const loaded = j.captures.slice().reverse()
